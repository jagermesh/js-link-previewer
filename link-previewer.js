function LinkPreviewer() {
  ...
  const previewerClass = 'link-previewer';
  const styleClass = `${previewerClass}-styles`;
  ...
  if (stylesContainer.length === 0) {
    ...
    stylesContainer.textContent = `
      ...
      .${previewerClass}-image-loading {
        height:190px;
        width:100%;
        text-align:center;
      }
      .${previewerClass}-image-loading div {
        padding:10px;
        padding-top:34px;
      }
      .${previewerClass}-image-content {
        width:100%;
        padding:5px;
      }
      .${previewerClass}-drawing {
      }
      .${previewerClass}-text {
        min-height:190px;
        max-height:300px;
      ...
      .${previewerClass}-video {
        width:100%;
      }
      .${previewerClass}-unknown-format {
        height:190px;
        width:100%;
        text-align:center;
        color:black;
      ...
      .${previewerClass}-unknown-format h2 {
        font-size:22pt;
        margin-top:20px;
      }
      .${previewerClass}-unknown-format h4 {
        font-size:18pt;
        margin-top:20px;
      }
      .${previewerClass}-show {
  ...
  const previewerOverlayTemplate = `
    <div class="${previewerClass}-image-loading" style="display:none;">
      <div>
    ...
    <img class="${previewerClass}-image-content ${previewerClass}-content" style="display:none;" />
    <div class="${previewerClass}-drawing ${previewerClass}-content" style="display:none;">
      <canvas class="${previewerClass}-drawing-canvas"></canvas>
    </div>
    <pre class="${previewerClass}-text ${previewerClass}-content" style="display:none;"></pre>
    <div class="${previewerClass}-html ${previewerClass}-content" style="display:none;"></div>
    <video class="${previewerClass}-video ${previewerClass}-content" autoplay muted loop style="display:none;"></video>
    <div class="${previewerClass}-unknown-format" style="display:none;">
      <h2>Preview not available</h2>
      <h4>Unfortunately we cannot show preview<br />for this file type</h4>
    </div>
  `;
  ...
  function Renderer(selector, event, params) {
    ...
    previewerOverlay.classList.add(`${previewerClass}-container`);
    previewerOverlay.classList.add(`${previewerClass}-hide`);
    ...
    const contentImage = previewerOverlay.querySelector(`img.${previewerClass}-image-content`);
    ...
    const contentVideo = previewerOverlay.querySelector(`video.${previewerClass}-video`);
    ...
    const contentText = previewerOverlay.querySelector(`pre.${previewerClass}-text`);
    // const contentHtml = previewerOverlay.querySelector(`div.${previewerClass}-html`);
    const contentDrawing = previewerOverlay.querySelector(`div.${previewerClass}-drawing`);
    const contentDrawingCanvas = contentDrawing.querySelector(`canvas.${previewerClass}-drawing-canvas`);
    const contentDrawingContext = contentDrawingCanvas.getContext('2d');
    const loadingImage = previewerOverlay.querySelector(`div.${previewerClass}-image-loading`);
    const unknownFormat = previewerOverlay.querySelector(`div.${previewerClass}-unknown-format`);
    ...
    saveMousePos(event);
    reposition();
    previewerOverlay.classList.add(`${previewerClass}-show`);
    previewerOverlay.classList.remove(`${previewerClass}-hide`);
  ...
  function createPreviewRenderer(element, event, params) {
    if (!element.classList.contains(previewerClass)) {
      element.classList.add(previewerClass);
    }
    if (activeRenderer) {
    ...
    element.setAttribute(`${previewerClass}-attached`, true);
  }
  ...
  function destroyPreviewRenderer(element) {
    ...
    if (element) {
      element.removeAttribute(`${previewerClass}-attached`);
    }
  }
